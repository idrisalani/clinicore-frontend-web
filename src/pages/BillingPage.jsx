...
import {
  Plus, Search, Lock, DollarSign, TrendingUp,
  FileText, CheckCircle, Edit2, Trash2, CreditCard, Shield,
} from 'lucide-react';
  ...
  return (
    <div className="flex justify-center items-center gap-2 py-4 border-t border-slate-100">
      <button disabled={currentPage<=1} onClick={() => setCurrentPage(p=>p-1)}
        className="px-4 py-2 text-sm font-semibold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 disabled:opacity-40 transition-all">
        ← Prev
      </button>
      {Array.from({length:Math.min(pagination.totalPages,5)},(_,i)=>i+1).map(pg => (
        <button key={pg} onClick={() => setCurrentPage(pg)}
          className={`w-9 h-9 rounded-xl text-sm font-bold transition-all ${pg===currentPage?'bg-teal-600 text-white':'text-slate-600 hover:bg-slate-100'}`}>
          {pg}
        </button>
      ))}
...
const BillingPage = () => {
  ...
  const handleInvoiceSubmit = async (formData) => {
    try {
      ...
      fetchInvoices(1); fetchStats(); setCurrentPage(1);
  ...
  const handlePaymentSubmit = async (formData) => {
    try {
      ...
      fetchInvoices(currentPage); fetchStats();
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between flex-wrap gap-4">
          <div className="fade-in">
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">
              {isPatient ? 'My Bills' : 'Billing & Invoicing'}
            </h1>
            <p className="text-slate-400 text-sm mt-0.5">
              {isPatient ? 'Your invoices and payment history' : 'Manage invoices, payments & insurance claims (₦)'}
            </p>
          </div>
          {p.canCreate && activeTab === 'invoices' && (
            <button
              onClick={() => { setSelectedInvoice(null); setModalMode('add'); setShowInvoiceModal(true); }}
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-5">
        ...
        {!p.canCreate && !p.canEdit && (
          <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3">
            ...
            <p className="text-sm text-amber-700 font-medium">
              {isPatient ? 'Your invoices — read only' : 'View only'}
            </p>
          </div>
        )}
                      ...
                      <thead>
                        <tr className="border-b border-slate-100">
                          {['Invoice #', !isPatient&&'Patient','Date','Amount','Paid','Due','Status',''].filter(Boolean).map((h,i) => (
                            <th key={i} className={`px-5 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider ${
                              ['Amount','Paid','Due',''].includes(h)?'text-right':'text-left'}`}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {invoices.map((inv, idx) => (
                          <tr key={inv.invoice_id} className={`border-b border-slate-50 hover:bg-teal-50/40 transition-colors ${idx%2===0?'bg-white':'bg-slate-50/50'}`}>
                            ...
                            <td className="px-5 py-4 text-sm text-slate-500">
                              {new Date(inv.invoice_date).toLocaleDateString('en-NG',{day:'numeric',month:'short',year:'numeric'})}
                            </td>
                            ...
                            <td className="px-5 py-4">
                              <span className={`inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full ${STATUS_BADGE[inv.status]||'bg-slate-100 text-slate-600'}`}>
                                {inv.status}
                              </span>
                            </td>
                            <td className="px-5 py-4">
        ...
        {activeTab==='insurance' && !isPatient && (
          <div className="fade-in">
            <InsuranceClaimsTab invoices={invoices}/>
          </div>
        )}
      </div>
