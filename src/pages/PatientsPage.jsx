...
const PatientsPage = () => {
  ...
  }, [p.isBlocked, isPatient, userId, showToast]);
  ...
  const handleSearch = useCallback(async (e) => {
    ...
    if (q.length >= 2) {
      try {
        ...
        setPatients(isPatient && userId
          ? results.filter(pt => pt.user_id === userId || pt.patient_id === userId)
          : results);
        setPagination({});
      } catch {}
    } else if (q.length === 0) {
  ...
  // ── FIX: await fetchPatients + show specific error, don't close modal on error ──
  const handleFormSubmit = async (formData) => {
    try {
      ...
      setCurrentPage(1);
      await fetchPatients(1, '');          // ← await so list refreshes immediately
    } catch (err) {
      // Show the actual backend error so admin can fix it
      const msg = err?.response?.data?.error || err?.message || 'Failed to save patient';
      showToast(msg, 'error');
      // Don't close modal — let admin see the error and retry
    } finally {
      setIsSubmitting(false);
    }
  };

  // ── FIX: await fetchPatients after delete too ─────────────────────────────────
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      await fetchPatients(currentPage, searchQuery);   // ← await
    } catch {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {p.canCreate && (
            <button
              onClick={() => { setSelectedPatient(null); setModalMode('add'); setShowModal(true); }}
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 fade-in">
          <StatCard icon={Users}      label={isPatient ? 'Your Records' : 'Total Patients'} value={isPatient ? 1 : (pagination.total || patients.length)} color="bg-teal-500" />
          <StatCard icon={TrendingUp} label="This Month"   value={pagination.newThisMonth || 0} color="bg-blue-500" />
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl
                  focus:outline-none focus:border-teal-400 focus:bg-white focus:ring-2 focus:ring-teal-100 transition-all"
              />
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <PatientTable
            ...
            isLoading={isLoading}
            onEdit={p.canEdit ? (id) => {
              const pt = patients.find(x => x.patient_id === id);
              setSelectedPatient(pt); setModalMode('edit'); setShowModal(true);
            } : null}
      ...
      {(p.canCreate || p.canEdit) && (
        ...
        >
          <PatientForm
            ...
            onCancel={() => { setShowModal(false); setSelectedPatient(null); }}
          />
        </Modal>
      )}
    </div>
